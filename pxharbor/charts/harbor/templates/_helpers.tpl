...
{{- define "harbor.redis.pwdfromsecret" -}}
  ...
  {{- if and (not (empty $existingSecret)) (hasKey $existingSecret.data "REDIS_PASSWORD") -}}
    {{- $existingSecret.data.REDIS_PASSWORD | b64dec -}}
  {{- end -}}
{{- end -}}
...
{{- define "harbor.redis.cred" -}}
  {{- with .Values.redis }}
    {{- if (and (eq .type "external" ) (.external.existingSecret)) }}
      {{- $username := (include "harbor.redis.usernamefromsecret" $) -}}
      {{- if not $username -}}{{- $username = (.external.username | default "") -}}{{- end -}}
      {{- printf "%s:%s@" $username (include "harbor.redis.pwdfromsecret" $) -}}
    {{- else }}
